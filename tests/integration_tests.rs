...
fn test_partitioned_contraction_random() {
    ...
    let r_tn = random_connected_circuit(k, 10, 0.5, 0.5, &mut rng, ConnectivityLayout::Eagle);
...
fn test_partitioned_contraction() {
    ...
    let r_tn = random_connected_circuit(k, 10, 0.5, 0.5, &mut rng, ConnectivityLayout::Osprey);
    let mut ref_tn = r_tn.clone();
    let mut ref_opt = Greedy::new(&ref_tn, CostType::Flops);
...
fn test_partitioned_contraction_mixed() {
    ...
    let r_tn = random_connected_circuit(k, 10, 0.5, 0.5, &mut rng, ConnectivityLayout::Condor);
    let mut ref_tn = r_tn.clone();
    let mut ref_opt = Greedy::new(&ref_tn, CostType::Flops);
    ref_opt.optimize_path();
    let ref_path = ref_opt.get_best_replace_path();
...
fn test_partitioned_contraction_need_mpi() {
    ...
    let (mut ref_tn, partitioned_tn, path) = if rank == 0 {
        let k = 10;
        let r_tn = random_connected_circuit(k, 10, 0.4, 0.4, &mut rng, ConnectivityLayout::Osprey);

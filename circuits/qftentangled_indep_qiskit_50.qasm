OPENQASM 2.0;
include "qelib1.inc";
qreg q[50];
creg meas[50];
h q[49];
cx q[49],q[48];
cx q[48],q[47];
cx q[47],q[46];
cx q[46],q[45];
cx q[45],q[44];
cx q[44],q[43];
cx q[43],q[42];
cx q[42],q[41];
cx q[41],q[40];
cx q[40],q[39];
cx q[39],q[38];
cx q[38],q[37];
cx q[37],q[36];
cx q[36],q[35];
cx q[35],q[34];
cx q[34],q[33];
cx q[33],q[32];
cx q[32],q[31];
cx q[31],q[30];
cx q[30],q[29];
cx q[29],q[28];
cx q[28],q[27];
cx q[27],q[26];
cx q[26],q[25];
cx q[25],q[24];
cx q[24],q[23];
cx q[23],q[22];
cx q[22],q[21];
cx q[21],q[20];
cx q[20],q[19];
cx q[19],q[18];
cx q[18],q[17];
cx q[17],q[16];
cx q[16],q[15];
cx q[15],q[14];
cx q[14],q[13];
cx q[13],q[12];
cx q[12],q[11];
cx q[11],q[10];
cx q[10],q[9];
cx q[9],q[8];
cx q[8],q[7];
cx q[7],q[6];
cx q[6],q[5];
cx q[5],q[4];
cx q[4],q[3];
cx q[3],q[2];
cx q[2],q[1];
cx q[1],q[0];
h q[49];
cp(pi/2) q[49],q[48];
h q[48];
cp(pi/4) q[49],q[47];
cp(pi/2) q[48],q[47];
h q[47];
cp(pi/8) q[49],q[46];
cp(pi/4) q[48],q[46];
cp(pi/2) q[47],q[46];
h q[46];
cp(pi/16) q[49],q[45];
cp(pi/8) q[48],q[45];
cp(pi/4) q[47],q[45];
cp(pi/2) q[46],q[45];
h q[45];
cp(pi/32) q[49],q[44];
cp(pi/16) q[48],q[44];
cp(pi/8) q[47],q[44];
cp(pi/4) q[46],q[44];
cp(pi/2) q[45],q[44];
h q[44];
cp(pi/64) q[49],q[43];
cp(pi/32) q[48],q[43];
cp(pi/16) q[47],q[43];
cp(pi/8) q[46],q[43];
cp(pi/4) q[45],q[43];
cp(pi/2) q[44],q[43];
h q[43];
cp(pi/128) q[49],q[42];
cp(pi/64) q[48],q[42];
cp(pi/32) q[47],q[42];
cp(pi/16) q[46],q[42];
cp(pi/8) q[45],q[42];
cp(pi/4) q[44],q[42];
cp(pi/2) q[43],q[42];
h q[42];
cp(pi/256) q[49],q[41];
cp(pi/128) q[48],q[41];
cp(pi/64) q[47],q[41];
cp(pi/32) q[46],q[41];
cp(pi/16) q[45],q[41];
cp(pi/8) q[44],q[41];
cp(pi/4) q[43],q[41];
cp(pi/2) q[42],q[41];
h q[41];
cp(pi/512) q[49],q[40];
cp(pi/256) q[48],q[40];
cp(pi/128) q[47],q[40];
cp(pi/64) q[46],q[40];
cp(pi/32) q[45],q[40];
cp(pi/16) q[44],q[40];
cp(pi/8) q[43],q[40];
cp(pi/4) q[42],q[40];
cp(pi/2) q[41],q[40];
h q[40];
cp(pi/1024) q[49],q[39];
cp(pi/512) q[48],q[39];
cp(pi/256) q[47],q[39];
cp(pi/128) q[46],q[39];
cp(pi/64) q[45],q[39];
cp(pi/32) q[44],q[39];
cp(pi/16) q[43],q[39];
cp(pi/8) q[42],q[39];
cp(pi/4) q[41],q[39];
cp(pi/2) q[40],q[39];
h q[39];
cp(pi/2048) q[49],q[38];
cp(pi/1024) q[48],q[38];
cp(pi/512) q[47],q[38];
cp(pi/256) q[46],q[38];
cp(pi/128) q[45],q[38];
cp(pi/64) q[44],q[38];
cp(pi/32) q[43],q[38];
cp(pi/16) q[42],q[38];
cp(pi/8) q[41],q[38];
cp(pi/4) q[40],q[38];
cp(pi/2) q[39],q[38];
h q[38];
cp(pi/4096) q[49],q[37];
cp(pi/2048) q[48],q[37];
cp(pi/1024) q[47],q[37];
cp(pi/512) q[46],q[37];
cp(pi/256) q[45],q[37];
cp(pi/128) q[44],q[37];
cp(pi/64) q[43],q[37];
cp(pi/32) q[42],q[37];
cp(pi/16) q[41],q[37];
cp(pi/8) q[40],q[37];
cp(pi/4) q[39],q[37];
cp(pi/2) q[38],q[37];
h q[37];
cp(pi/8192) q[49],q[36];
cp(pi/4096) q[48],q[36];
cp(pi/2048) q[47],q[36];
cp(pi/1024) q[46],q[36];
cp(pi/512) q[45],q[36];
cp(pi/256) q[44],q[36];
cp(pi/128) q[43],q[36];
cp(pi/64) q[42],q[36];
cp(pi/32) q[41],q[36];
cp(pi/16) q[40],q[36];
cp(pi/8) q[39],q[36];
cp(pi/4) q[38],q[36];
cp(pi/2) q[37],q[36];
h q[36];
cp(pi/16384) q[49],q[35];
cp(pi/8192) q[48],q[35];
cp(pi/4096) q[47],q[35];
cp(pi/2048) q[46],q[35];
cp(pi/1024) q[45],q[35];
cp(pi/512) q[44],q[35];
cp(pi/256) q[43],q[35];
cp(pi/128) q[42],q[35];
cp(pi/64) q[41],q[35];
cp(pi/32) q[40],q[35];
cp(pi/16) q[39],q[35];
cp(pi/8) q[38],q[35];
cp(pi/4) q[37],q[35];
cp(pi/2) q[36],q[35];
h q[35];
cp(pi/32768) q[49],q[34];
cp(pi/16384) q[48],q[34];
cp(pi/8192) q[47],q[34];
cp(pi/4096) q[46],q[34];
cp(pi/2048) q[45],q[34];
cp(pi/1024) q[44],q[34];
cp(pi/512) q[43],q[34];
cp(pi/256) q[42],q[34];
cp(pi/128) q[41],q[34];
cp(pi/64) q[40],q[34];
cp(pi/32) q[39],q[34];
cp(pi/16) q[38],q[34];
cp(pi/8) q[37],q[34];
cp(pi/4) q[36],q[34];
cp(pi/2) q[35],q[34];
h q[34];
cp(pi/65536) q[49],q[33];
cp(pi/32768) q[48],q[33];
cp(pi/16384) q[47],q[33];
cp(pi/8192) q[46],q[33];
cp(pi/4096) q[45],q[33];
cp(pi/2048) q[44],q[33];
cp(pi/1024) q[43],q[33];
cp(pi/512) q[42],q[33];
cp(pi/256) q[41],q[33];
cp(pi/128) q[40],q[33];
cp(pi/64) q[39],q[33];
cp(pi/32) q[38],q[33];
cp(pi/16) q[37],q[33];
cp(pi/8) q[36],q[33];
cp(pi/4) q[35],q[33];
cp(pi/2) q[34],q[33];
h q[33];
cp(pi/131072) q[49],q[32];
cp(pi/65536) q[48],q[32];
cp(pi/32768) q[47],q[32];
cp(pi/16384) q[46],q[32];
cp(pi/8192) q[45],q[32];
cp(pi/4096) q[44],q[32];
cp(pi/2048) q[43],q[32];
cp(pi/1024) q[42],q[32];
cp(pi/512) q[41],q[32];
cp(pi/256) q[40],q[32];
cp(pi/128) q[39],q[32];
cp(pi/64) q[38],q[32];
cp(pi/32) q[37],q[32];
cp(pi/16) q[36],q[32];
cp(pi/8) q[35],q[32];
cp(pi/4) q[34],q[32];
cp(pi/2) q[33],q[32];
h q[32];
cp(pi/262144) q[49],q[31];
cp(pi/131072) q[48],q[31];
cp(pi/65536) q[47],q[31];
cp(pi/32768) q[46],q[31];
cp(pi/16384) q[45],q[31];
cp(pi/8192) q[44],q[31];
cp(pi/4096) q[43],q[31];
cp(pi/2048) q[42],q[31];
cp(pi/1024) q[41],q[31];
cp(pi/512) q[40],q[31];
cp(pi/256) q[39],q[31];
cp(pi/128) q[38],q[31];
cp(pi/64) q[37],q[31];
cp(pi/32) q[36],q[31];
cp(pi/16) q[35],q[31];
cp(pi/8) q[34],q[31];
cp(pi/4) q[33],q[31];
cp(pi/2) q[32],q[31];
h q[31];
cp(pi/524288) q[49],q[30];
cp(pi/262144) q[48],q[30];
cp(pi/131072) q[47],q[30];
cp(pi/65536) q[46],q[30];
cp(pi/32768) q[45],q[30];
cp(pi/16384) q[44],q[30];
cp(pi/8192) q[43],q[30];
cp(pi/4096) q[42],q[30];
cp(pi/2048) q[41],q[30];
cp(pi/1024) q[40],q[30];
cp(pi/512) q[39],q[30];
cp(pi/256) q[38],q[30];
cp(pi/128) q[37],q[30];
cp(pi/64) q[36],q[30];
cp(pi/32) q[35],q[30];
cp(pi/16) q[34],q[30];
cp(pi/8) q[33],q[30];
cp(pi/4) q[32],q[30];
cp(pi/2) q[31],q[30];
h q[30];
cp(pi/1048576) q[49],q[29];
cp(pi/524288) q[48],q[29];
cp(pi/262144) q[47],q[29];
cp(pi/131072) q[46],q[29];
cp(pi/65536) q[45],q[29];
cp(pi/32768) q[44],q[29];
cp(pi/16384) q[43],q[29];
cp(pi/8192) q[42],q[29];
cp(pi/4096) q[41],q[29];
cp(pi/2048) q[40],q[29];
cp(pi/1024) q[39],q[29];
cp(pi/512) q[38],q[29];
cp(pi/256) q[37],q[29];
cp(pi/128) q[36],q[29];
cp(pi/64) q[35],q[29];
cp(pi/32) q[34],q[29];
cp(pi/16) q[33],q[29];
cp(pi/8) q[32],q[29];
cp(pi/4) q[31],q[29];
cp(pi/2) q[30],q[29];
h q[29];
cp(pi/2097152) q[49],q[28];
cp(pi/1048576) q[48],q[28];
cp(pi/524288) q[47],q[28];
cp(pi/262144) q[46],q[28];
cp(pi/131072) q[45],q[28];
cp(pi/65536) q[44],q[28];
cp(pi/32768) q[43],q[28];
cp(pi/16384) q[42],q[28];
cp(pi/8192) q[41],q[28];
cp(pi/4096) q[40],q[28];
cp(pi/2048) q[39],q[28];
cp(pi/1024) q[38],q[28];
cp(pi/512) q[37],q[28];
cp(pi/256) q[36],q[28];
cp(pi/128) q[35],q[28];
cp(pi/64) q[34],q[28];
cp(pi/32) q[33],q[28];
cp(pi/16) q[32],q[28];
cp(pi/8) q[31],q[28];
cp(pi/4) q[30],q[28];
cp(pi/2) q[29],q[28];
h q[28];
cp(pi/4194304) q[49],q[27];
cp(pi/2097152) q[48],q[27];
cp(pi/1048576) q[47],q[27];
cp(pi/524288) q[46],q[27];
cp(pi/262144) q[45],q[27];
cp(pi/131072) q[44],q[27];
cp(pi/65536) q[43],q[27];
cp(pi/32768) q[42],q[27];
cp(pi/16384) q[41],q[27];
cp(pi/8192) q[40],q[27];
cp(pi/4096) q[39],q[27];
cp(pi/2048) q[38],q[27];
cp(pi/1024) q[37],q[27];
cp(pi/512) q[36],q[27];
cp(pi/256) q[35],q[27];
cp(pi/128) q[34],q[27];
cp(pi/64) q[33],q[27];
cp(pi/32) q[32],q[27];
cp(pi/16) q[31],q[27];
cp(pi/8) q[30],q[27];
cp(pi/4) q[29],q[27];
cp(pi/2) q[28],q[27];
h q[27];
cp(pi/8388608) q[49],q[26];
cp(pi/4194304) q[48],q[26];
cp(pi/2097152) q[47],q[26];
cp(pi/1048576) q[46],q[26];
cp(pi/524288) q[45],q[26];
cp(pi/262144) q[44],q[26];
cp(pi/131072) q[43],q[26];
cp(pi/65536) q[42],q[26];
cp(pi/32768) q[41],q[26];
cp(pi/16384) q[40],q[26];
cp(pi/8192) q[39],q[26];
cp(pi/4096) q[38],q[26];
cp(pi/2048) q[37],q[26];
cp(pi/1024) q[36],q[26];
cp(pi/512) q[35],q[26];
cp(pi/256) q[34],q[26];
cp(pi/128) q[33],q[26];
cp(pi/64) q[32],q[26];
cp(pi/32) q[31],q[26];
cp(pi/16) q[30],q[26];
cp(pi/8) q[29],q[26];
cp(pi/4) q[28],q[26];
cp(pi/2) q[27],q[26];
h q[26];
cp(pi/16777216) q[49],q[25];
cp(pi/8388608) q[48],q[25];
cp(pi/4194304) q[47],q[25];
cp(pi/2097152) q[46],q[25];
cp(pi/1048576) q[45],q[25];
cp(pi/524288) q[44],q[25];
cp(pi/262144) q[43],q[25];
cp(pi/131072) q[42],q[25];
cp(pi/65536) q[41],q[25];
cp(pi/32768) q[40],q[25];
cp(pi/16384) q[39],q[25];
cp(pi/8192) q[38],q[25];
cp(pi/4096) q[37],q[25];
cp(pi/2048) q[36],q[25];
cp(pi/1024) q[35],q[25];
cp(pi/512) q[34],q[25];
cp(pi/256) q[33],q[25];
cp(pi/128) q[32],q[25];
cp(pi/64) q[31],q[25];
cp(pi/32) q[30],q[25];
cp(pi/16) q[29],q[25];
cp(pi/8) q[28],q[25];
cp(pi/4) q[27],q[25];
cp(pi/2) q[26],q[25];
h q[25];
cp(pi/33554432) q[49],q[24];
cp(pi/16777216) q[48],q[24];
cp(pi/8388608) q[47],q[24];
cp(pi/4194304) q[46],q[24];
cp(pi/2097152) q[45],q[24];
cp(pi/1048576) q[44],q[24];
cp(pi/524288) q[43],q[24];
cp(pi/262144) q[42],q[24];
cp(pi/131072) q[41],q[24];
cp(pi/65536) q[40],q[24];
cp(pi/32768) q[39],q[24];
cp(pi/16384) q[38],q[24];
cp(pi/8192) q[37],q[24];
cp(pi/4096) q[36],q[24];
cp(pi/2048) q[35],q[24];
cp(pi/1024) q[34],q[24];
cp(pi/512) q[33],q[24];
cp(pi/256) q[32],q[24];
cp(pi/128) q[31],q[24];
cp(pi/64) q[30],q[24];
cp(pi/32) q[29],q[24];
cp(pi/16) q[28],q[24];
cp(pi/8) q[27],q[24];
cp(pi/4) q[26],q[24];
cp(pi/2) q[25],q[24];
h q[24];
cp(pi/67108864) q[49],q[23];
cp(pi/33554432) q[48],q[23];
cp(pi/16777216) q[47],q[23];
cp(pi/8388608) q[46],q[23];
cp(pi/4194304) q[45],q[23];
cp(pi/2097152) q[44],q[23];
cp(pi/1048576) q[43],q[23];
cp(pi/524288) q[42],q[23];
cp(pi/262144) q[41],q[23];
cp(pi/131072) q[40],q[23];
cp(pi/65536) q[39],q[23];
cp(pi/32768) q[38],q[23];
cp(pi/16384) q[37],q[23];
cp(pi/8192) q[36],q[23];
cp(pi/4096) q[35],q[23];
cp(pi/2048) q[34],q[23];
cp(pi/1024) q[33],q[23];
cp(pi/512) q[32],q[23];
cp(pi/256) q[31],q[23];
cp(pi/128) q[30],q[23];
cp(pi/64) q[29],q[23];
cp(pi/32) q[28],q[23];
cp(pi/16) q[27],q[23];
cp(pi/8) q[26],q[23];
cp(pi/4) q[25],q[23];
cp(pi/2) q[24],q[23];
h q[23];
cp(pi/134217728) q[49],q[22];
cp(pi/67108864) q[48],q[22];
cp(pi/33554432) q[47],q[22];
cp(pi/16777216) q[46],q[22];
cp(pi/8388608) q[45],q[22];
cp(pi/4194304) q[44],q[22];
cp(pi/2097152) q[43],q[22];
cp(pi/1048576) q[42],q[22];
cp(pi/524288) q[41],q[22];
cp(pi/262144) q[40],q[22];
cp(pi/131072) q[39],q[22];
cp(pi/65536) q[38],q[22];
cp(pi/32768) q[37],q[22];
cp(pi/16384) q[36],q[22];
cp(pi/8192) q[35],q[22];
cp(pi/4096) q[34],q[22];
cp(pi/2048) q[33],q[22];
cp(pi/1024) q[32],q[22];
cp(pi/512) q[31],q[22];
cp(pi/256) q[30],q[22];
cp(pi/128) q[29],q[22];
cp(pi/64) q[28],q[22];
cp(pi/32) q[27],q[22];
cp(pi/16) q[26],q[22];
cp(pi/8) q[25],q[22];
cp(pi/4) q[24],q[22];
cp(pi/2) q[23],q[22];
h q[22];
cp(pi/268435456) q[49],q[21];
cp(pi/134217728) q[48],q[21];
cp(pi/67108864) q[47],q[21];
cp(pi/33554432) q[46],q[21];
cp(pi/16777216) q[45],q[21];
cp(pi/8388608) q[44],q[21];
cp(pi/4194304) q[43],q[21];
cp(pi/2097152) q[42],q[21];
cp(pi/1048576) q[41],q[21];
cp(pi/524288) q[40],q[21];
cp(pi/262144) q[39],q[21];
cp(pi/131072) q[38],q[21];
cp(pi/65536) q[37],q[21];
cp(pi/32768) q[36],q[21];
cp(pi/16384) q[35],q[21];
cp(pi/8192) q[34],q[21];
cp(pi/4096) q[33],q[21];
cp(pi/2048) q[32],q[21];
cp(pi/1024) q[31],q[21];
cp(pi/512) q[30],q[21];
cp(pi/256) q[29],q[21];
cp(pi/128) q[28],q[21];
cp(pi/64) q[27],q[21];
cp(pi/32) q[26],q[21];
cp(pi/16) q[25],q[21];
cp(pi/8) q[24],q[21];
cp(pi/4) q[23],q[21];
cp(pi/2) q[22],q[21];
h q[21];
cp(pi/536870912) q[49],q[20];
cp(pi/268435456) q[48],q[20];
cp(pi/134217728) q[47],q[20];
cp(pi/67108864) q[46],q[20];
cp(pi/33554432) q[45],q[20];
cp(pi/16777216) q[44],q[20];
cp(pi/8388608) q[43],q[20];
cp(pi/4194304) q[42],q[20];
cp(pi/2097152) q[41],q[20];
cp(pi/1048576) q[40],q[20];
cp(pi/524288) q[39],q[20];
cp(pi/262144) q[38],q[20];
cp(pi/131072) q[37],q[20];
cp(pi/65536) q[36],q[20];
cp(pi/32768) q[35],q[20];
cp(pi/16384) q[34],q[20];
cp(pi/8192) q[33],q[20];
cp(pi/4096) q[32],q[20];
cp(pi/2048) q[31],q[20];
cp(pi/1024) q[30],q[20];
cp(pi/512) q[29],q[20];
cp(pi/256) q[28],q[20];
cp(pi/128) q[27],q[20];
cp(pi/64) q[26],q[20];
cp(pi/32) q[25],q[20];
cp(pi/16) q[24],q[20];
cp(pi/8) q[23],q[20];
cp(pi/4) q[22],q[20];
cp(pi/2) q[21],q[20];
h q[20];
cp(pi/1073741824) q[49],q[19];
cp(pi/536870912) q[48],q[19];
cp(pi/268435456) q[47],q[19];
cp(pi/134217728) q[46],q[19];
cp(pi/67108864) q[45],q[19];
cp(pi/33554432) q[44],q[19];
cp(pi/16777216) q[43],q[19];
cp(pi/8388608) q[42],q[19];
cp(pi/4194304) q[41],q[19];
cp(pi/2097152) q[40],q[19];
cp(pi/1048576) q[39],q[19];
cp(pi/524288) q[38],q[19];
cp(pi/262144) q[37],q[19];
cp(pi/131072) q[36],q[19];
cp(pi/65536) q[35],q[19];
cp(pi/32768) q[34],q[19];
cp(pi/16384) q[33],q[19];
cp(pi/8192) q[32],q[19];
cp(pi/4096) q[31],q[19];
cp(pi/2048) q[30],q[19];
cp(pi/1024) q[29],q[19];
cp(pi/512) q[28],q[19];
cp(pi/256) q[27],q[19];
cp(pi/128) q[26],q[19];
cp(pi/64) q[25],q[19];
cp(pi/32) q[24],q[19];
cp(pi/16) q[23],q[19];
cp(pi/8) q[22],q[19];
cp(pi/4) q[21],q[19];
cp(pi/2) q[20],q[19];
h q[19];
cp(pi/2147483648.0) q[49],q[18];
cp(pi/1073741824) q[48],q[18];
cp(pi/536870912) q[47],q[18];
cp(pi/268435456) q[46],q[18];
cp(pi/134217728) q[45],q[18];
cp(pi/67108864) q[44],q[18];
cp(pi/33554432) q[43],q[18];
cp(pi/16777216) q[42],q[18];
cp(pi/8388608) q[41],q[18];
cp(pi/4194304) q[40],q[18];
cp(pi/2097152) q[39],q[18];
cp(pi/1048576) q[38],q[18];
cp(pi/524288) q[37],q[18];
cp(pi/262144) q[36],q[18];
cp(pi/131072) q[35],q[18];
cp(pi/65536) q[34],q[18];
cp(pi/32768) q[33],q[18];
cp(pi/16384) q[32],q[18];
cp(pi/8192) q[31],q[18];
cp(pi/4096) q[30],q[18];
cp(pi/2048) q[29],q[18];
cp(pi/1024) q[28],q[18];
cp(pi/512) q[27],q[18];
cp(pi/256) q[26],q[18];
cp(pi/128) q[25],q[18];
cp(pi/64) q[24],q[18];
cp(pi/32) q[23],q[18];
cp(pi/16) q[22],q[18];
cp(pi/8) q[21],q[18];
cp(pi/4) q[20],q[18];
cp(pi/2) q[19],q[18];
h q[18];
cp(pi/4294967296.0) q[49],q[17];
cp(pi/2147483648.0) q[48],q[17];
cp(pi/1073741824) q[47],q[17];
cp(pi/536870912) q[46],q[17];
cp(pi/268435456) q[45],q[17];
cp(pi/134217728) q[44],q[17];
cp(pi/67108864) q[43],q[17];
cp(pi/33554432) q[42],q[17];
cp(pi/16777216) q[41],q[17];
cp(pi/8388608) q[40],q[17];
cp(pi/4194304) q[39],q[17];
cp(pi/2097152) q[38],q[17];
cp(pi/1048576) q[37],q[17];
cp(pi/524288) q[36],q[17];
cp(pi/262144) q[35],q[17];
cp(pi/131072) q[34],q[17];
cp(pi/65536) q[33],q[17];
cp(pi/32768) q[32],q[17];
cp(pi/16384) q[31],q[17];
cp(pi/8192) q[30],q[17];
cp(pi/4096) q[29],q[17];
cp(pi/2048) q[28],q[17];
cp(pi/1024) q[27],q[17];
cp(pi/512) q[26],q[17];
cp(pi/256) q[25],q[17];
cp(pi/128) q[24],q[17];
cp(pi/64) q[23],q[17];
cp(pi/32) q[22],q[17];
cp(pi/16) q[21],q[17];
cp(pi/8) q[20],q[17];
cp(pi/4) q[19],q[17];
cp(pi/2) q[18],q[17];
h q[17];
cp(pi/8589934592.0) q[49],q[16];
cp(pi/4294967296.0) q[48],q[16];
cp(pi/2147483648.0) q[47],q[16];
cp(pi/1073741824) q[46],q[16];
cp(pi/536870912) q[45],q[16];
cp(pi/268435456) q[44],q[16];
cp(pi/134217728) q[43],q[16];
cp(pi/67108864) q[42],q[16];
cp(pi/33554432) q[41],q[16];
cp(pi/16777216) q[40],q[16];
cp(pi/8388608) q[39],q[16];
cp(pi/4194304) q[38],q[16];
cp(pi/2097152) q[37],q[16];
cp(pi/1048576) q[36],q[16];
cp(pi/524288) q[35],q[16];
cp(pi/262144) q[34],q[16];
cp(pi/131072) q[33],q[16];
cp(pi/65536) q[32],q[16];
cp(pi/32768) q[31],q[16];
cp(pi/16384) q[30],q[16];
cp(pi/8192) q[29],q[16];
cp(pi/4096) q[28],q[16];
cp(pi/2048) q[27],q[16];
cp(pi/1024) q[26],q[16];
cp(pi/512) q[25],q[16];
cp(pi/256) q[24],q[16];
cp(pi/128) q[23],q[16];
cp(pi/64) q[22],q[16];
cp(pi/32) q[21],q[16];
cp(pi/16) q[20],q[16];
cp(pi/8) q[19],q[16];
cp(pi/4) q[18],q[16];
cp(pi/2) q[17],q[16];
h q[16];
cp(pi/17179869184.0) q[49],q[15];
cp(pi/8589934592.0) q[48],q[15];
cp(pi/4294967296.0) q[47],q[15];
cp(pi/2147483648.0) q[46],q[15];
cp(pi/1073741824) q[45],q[15];
cp(pi/536870912) q[44],q[15];
cp(pi/268435456) q[43],q[15];
cp(pi/134217728) q[42],q[15];
cp(pi/67108864) q[41],q[15];
cp(pi/33554432) q[40],q[15];
cp(pi/16777216) q[39],q[15];
cp(pi/8388608) q[38],q[15];
cp(pi/4194304) q[37],q[15];
cp(pi/2097152) q[36],q[15];
cp(pi/1048576) q[35],q[15];
cp(pi/524288) q[34],q[15];
cp(pi/262144) q[33],q[15];
cp(pi/131072) q[32],q[15];
cp(pi/65536) q[31],q[15];
cp(pi/32768) q[30],q[15];
cp(pi/16384) q[29],q[15];
cp(pi/8192) q[28],q[15];
cp(pi/4096) q[27],q[15];
cp(pi/2048) q[26],q[15];
cp(pi/1024) q[25],q[15];
cp(pi/512) q[24],q[15];
cp(pi/256) q[23],q[15];
cp(pi/128) q[22],q[15];
cp(pi/64) q[21],q[15];
cp(pi/32) q[20],q[15];
cp(pi/16) q[19],q[15];
cp(pi/8) q[18],q[15];
cp(pi/4) q[17],q[15];
cp(pi/2) q[16],q[15];
h q[15];
cp(pi/34359738368.0) q[49],q[14];
cp(pi/17179869184.0) q[48],q[14];
cp(pi/8589934592.0) q[47],q[14];
cp(pi/4294967296.0) q[46],q[14];
cp(pi/2147483648.0) q[45],q[14];
cp(pi/1073741824) q[44],q[14];
cp(pi/536870912) q[43],q[14];
cp(pi/268435456) q[42],q[14];
cp(pi/134217728) q[41],q[14];
cp(pi/67108864) q[40],q[14];
cp(pi/33554432) q[39],q[14];
cp(pi/16777216) q[38],q[14];
cp(pi/8388608) q[37],q[14];
cp(pi/4194304) q[36],q[14];
cp(pi/2097152) q[35],q[14];
cp(pi/1048576) q[34],q[14];
cp(pi/524288) q[33],q[14];
cp(pi/262144) q[32],q[14];
cp(pi/131072) q[31],q[14];
cp(pi/65536) q[30],q[14];
cp(pi/32768) q[29],q[14];
cp(pi/16384) q[28],q[14];
cp(pi/8192) q[27],q[14];
cp(pi/4096) q[26],q[14];
cp(pi/2048) q[25],q[14];
cp(pi/1024) q[24],q[14];
cp(pi/512) q[23],q[14];
cp(pi/256) q[22],q[14];
cp(pi/128) q[21],q[14];
cp(pi/64) q[20],q[14];
cp(pi/32) q[19],q[14];
cp(pi/16) q[18],q[14];
cp(pi/8) q[17],q[14];
cp(pi/4) q[16],q[14];
cp(pi/2) q[15],q[14];
h q[14];
cp(pi/68719476736.0) q[49],q[13];
cp(pi/34359738368.0) q[48],q[13];
cp(pi/17179869184.0) q[47],q[13];
cp(pi/8589934592.0) q[46],q[13];
cp(pi/4294967296.0) q[45],q[13];
cp(pi/2147483648.0) q[44],q[13];
cp(pi/1073741824) q[43],q[13];
cp(pi/536870912) q[42],q[13];
cp(pi/268435456) q[41],q[13];
cp(pi/134217728) q[40],q[13];
cp(pi/67108864) q[39],q[13];
cp(pi/33554432) q[38],q[13];
cp(pi/16777216) q[37],q[13];
cp(pi/8388608) q[36],q[13];
cp(pi/4194304) q[35],q[13];
cp(pi/2097152) q[34],q[13];
cp(pi/1048576) q[33],q[13];
cp(pi/524288) q[32],q[13];
cp(pi/262144) q[31],q[13];
cp(pi/131072) q[30],q[13];
cp(pi/65536) q[29],q[13];
cp(pi/32768) q[28],q[13];
cp(pi/16384) q[27],q[13];
cp(pi/8192) q[26],q[13];
cp(pi/4096) q[25],q[13];
cp(pi/2048) q[24],q[13];
cp(pi/1024) q[23],q[13];
cp(pi/512) q[22],q[13];
cp(pi/256) q[21],q[13];
cp(pi/128) q[20],q[13];
cp(pi/64) q[19],q[13];
cp(pi/32) q[18],q[13];
cp(pi/16) q[17],q[13];
cp(pi/8) q[16],q[13];
cp(pi/4) q[15],q[13];
cp(pi/2) q[14],q[13];
h q[13];
cp(pi/137438953472.0) q[49],q[12];
cp(pi/68719476736.0) q[48],q[12];
cp(pi/34359738368.0) q[47],q[12];
cp(pi/17179869184.0) q[46],q[12];
cp(pi/8589934592.0) q[45],q[12];
cp(pi/4294967296.0) q[44],q[12];
cp(pi/2147483648.0) q[43],q[12];
cp(pi/1073741824) q[42],q[12];
cp(pi/536870912) q[41],q[12];
cp(pi/268435456) q[40],q[12];
cp(pi/134217728) q[39],q[12];
cp(pi/67108864) q[38],q[12];
cp(pi/33554432) q[37],q[12];
cp(pi/16777216) q[36],q[12];
cp(pi/8388608) q[35],q[12];
cp(pi/4194304) q[34],q[12];
cp(pi/2097152) q[33],q[12];
cp(pi/1048576) q[32],q[12];
cp(pi/524288) q[31],q[12];
cp(pi/262144) q[30],q[12];
cp(pi/131072) q[29],q[12];
cp(pi/65536) q[28],q[12];
cp(pi/32768) q[27],q[12];
cp(pi/16384) q[26],q[12];
cp(pi/8192) q[25],q[12];
cp(pi/4096) q[24],q[12];
cp(pi/2048) q[23],q[12];
cp(pi/1024) q[22],q[12];
cp(pi/512) q[21],q[12];
cp(pi/256) q[20],q[12];
cp(pi/128) q[19],q[12];
cp(pi/64) q[18],q[12];
cp(pi/32) q[17],q[12];
cp(pi/16) q[16],q[12];
cp(pi/8) q[15],q[12];
cp(pi/4) q[14],q[12];
cp(pi/2) q[13],q[12];
h q[12];
cp(pi/274877906944.0) q[49],q[11];
cp(pi/137438953472.0) q[48],q[11];
cp(pi/68719476736.0) q[47],q[11];
cp(pi/34359738368.0) q[46],q[11];
cp(pi/17179869184.0) q[45],q[11];
cp(pi/8589934592.0) q[44],q[11];
cp(pi/4294967296.0) q[43],q[11];
cp(pi/2147483648.0) q[42],q[11];
cp(pi/1073741824) q[41],q[11];
cp(pi/536870912) q[40],q[11];
cp(pi/268435456) q[39],q[11];
cp(pi/134217728) q[38],q[11];
cp(pi/67108864) q[37],q[11];
cp(pi/33554432) q[36],q[11];
cp(pi/16777216) q[35],q[11];
cp(pi/8388608) q[34],q[11];
cp(pi/4194304) q[33],q[11];
cp(pi/2097152) q[32],q[11];
cp(pi/1048576) q[31],q[11];
cp(pi/524288) q[30],q[11];
cp(pi/262144) q[29],q[11];
cp(pi/131072) q[28],q[11];
cp(pi/65536) q[27],q[11];
cp(pi/32768) q[26],q[11];
cp(pi/16384) q[25],q[11];
cp(pi/8192) q[24],q[11];
cp(pi/4096) q[23],q[11];
cp(pi/2048) q[22],q[11];
cp(pi/1024) q[21],q[11];
cp(pi/512) q[20],q[11];
cp(pi/256) q[19],q[11];
cp(pi/128) q[18],q[11];
cp(pi/64) q[17],q[11];
cp(pi/32) q[16],q[11];
cp(pi/16) q[15],q[11];
cp(pi/8) q[14],q[11];
cp(pi/4) q[13],q[11];
cp(pi/2) q[12],q[11];
h q[11];
cp(pi/549755813888.0) q[49],q[10];
cp(pi/274877906944.0) q[48],q[10];
cp(pi/137438953472.0) q[47],q[10];
cp(pi/68719476736.0) q[46],q[10];
cp(pi/34359738368.0) q[45],q[10];
cp(pi/17179869184.0) q[44],q[10];
cp(pi/8589934592.0) q[43],q[10];
cp(pi/4294967296.0) q[42],q[10];
cp(pi/2147483648.0) q[41],q[10];
cp(pi/1073741824) q[40],q[10];
cp(pi/536870912) q[39],q[10];
cp(pi/268435456) q[38],q[10];
cp(pi/134217728) q[37],q[10];
cp(pi/67108864) q[36],q[10];
cp(pi/33554432) q[35],q[10];
cp(pi/16777216) q[34],q[10];
cp(pi/8388608) q[33],q[10];
cp(pi/4194304) q[32],q[10];
cp(pi/2097152) q[31],q[10];
cp(pi/1048576) q[30],q[10];
cp(pi/524288) q[29],q[10];
cp(pi/262144) q[28],q[10];
cp(pi/131072) q[27],q[10];
cp(pi/65536) q[26],q[10];
cp(pi/32768) q[25],q[10];
cp(pi/16384) q[24],q[10];
cp(pi/8192) q[23],q[10];
cp(pi/4096) q[22],q[10];
cp(pi/2048) q[21],q[10];
cp(pi/1024) q[20],q[10];
cp(pi/512) q[19],q[10];
cp(pi/256) q[18],q[10];
cp(pi/128) q[17],q[10];
cp(pi/64) q[16],q[10];
cp(pi/32) q[15],q[10];
cp(pi/16) q[14],q[10];
cp(pi/8) q[13],q[10];
cp(pi/4) q[12],q[10];
cp(pi/2) q[11],q[10];
h q[10];
cp(pi/1099511627776.0) q[49],q[9];
cp(pi/549755813888.0) q[48],q[9];
cp(pi/274877906944.0) q[47],q[9];
cp(pi/137438953472.0) q[46],q[9];
cp(pi/68719476736.0) q[45],q[9];
cp(pi/34359738368.0) q[44],q[9];
cp(pi/17179869184.0) q[43],q[9];
cp(pi/8589934592.0) q[42],q[9];
cp(pi/4294967296.0) q[41],q[9];
cp(pi/2147483648.0) q[40],q[9];
cp(pi/1073741824) q[39],q[9];
cp(pi/536870912) q[38],q[9];
cp(pi/268435456) q[37],q[9];
cp(pi/134217728) q[36],q[9];
cp(pi/67108864) q[35],q[9];
cp(pi/33554432) q[34],q[9];
cp(pi/16777216) q[33],q[9];
cp(pi/8388608) q[32],q[9];
cp(pi/4194304) q[31],q[9];
cp(pi/2097152) q[30],q[9];
cp(pi/1048576) q[29],q[9];
cp(pi/524288) q[28],q[9];
cp(pi/262144) q[27],q[9];
cp(pi/131072) q[26],q[9];
cp(pi/65536) q[25],q[9];
cp(pi/32768) q[24],q[9];
cp(pi/16384) q[23],q[9];
cp(pi/8192) q[22],q[9];
cp(pi/4096) q[21],q[9];
cp(pi/2048) q[20],q[9];
cp(pi/1024) q[19],q[9];
cp(pi/512) q[18],q[9];
cp(pi/256) q[17],q[9];
cp(pi/128) q[16],q[9];
cp(pi/64) q[15],q[9];
cp(pi/32) q[14],q[9];
cp(pi/16) q[13],q[9];
cp(pi/8) q[12],q[9];
cp(pi/4) q[11],q[9];
cp(pi/2) q[10],q[9];
h q[9];
cp(pi/2199023255552.0) q[49],q[8];
cp(pi/1099511627776.0) q[48],q[8];
cp(pi/549755813888.0) q[47],q[8];
cp(pi/274877906944.0) q[46],q[8];
cp(pi/137438953472.0) q[45],q[8];
cp(pi/68719476736.0) q[44],q[8];
cp(pi/34359738368.0) q[43],q[8];
cp(pi/17179869184.0) q[42],q[8];
cp(pi/8589934592.0) q[41],q[8];
cp(pi/4294967296.0) q[40],q[8];
cp(pi/2147483648.0) q[39],q[8];
cp(pi/1073741824) q[38],q[8];
cp(pi/536870912) q[37],q[8];
cp(pi/268435456) q[36],q[8];
cp(pi/134217728) q[35],q[8];
cp(pi/67108864) q[34],q[8];
cp(pi/33554432) q[33],q[8];
cp(pi/16777216) q[32],q[8];
cp(pi/8388608) q[31],q[8];
cp(pi/4194304) q[30],q[8];
cp(pi/2097152) q[29],q[8];
cp(pi/1048576) q[28],q[8];
cp(pi/524288) q[27],q[8];
cp(pi/262144) q[26],q[8];
cp(pi/131072) q[25],q[8];
cp(pi/65536) q[24],q[8];
cp(pi/32768) q[23],q[8];
cp(pi/16384) q[22],q[8];
cp(pi/8192) q[21],q[8];
cp(pi/4096) q[20],q[8];
cp(pi/2048) q[19],q[8];
cp(pi/1024) q[18],q[8];
cp(pi/512) q[17],q[8];
cp(pi/256) q[16],q[8];
cp(pi/128) q[15],q[8];
cp(pi/64) q[14],q[8];
cp(pi/32) q[13],q[8];
cp(pi/16) q[12],q[8];
cp(pi/8) q[11],q[8];
cp(pi/4) q[10],q[8];
cp(pi/2) q[9],q[8];
h q[8];
cp(0) q[49],q[7];
cp(pi/2199023255552.0) q[48],q[7];
cp(pi/1099511627776.0) q[47],q[7];
cp(pi/549755813888.0) q[46],q[7];
cp(pi/274877906944.0) q[45],q[7];
cp(pi/137438953472.0) q[44],q[7];
cp(pi/68719476736.0) q[43],q[7];
cp(pi/34359738368.0) q[42],q[7];
cp(pi/17179869184.0) q[41],q[7];
cp(pi/8589934592.0) q[40],q[7];
cp(pi/4294967296.0) q[39],q[7];
cp(pi/2147483648.0) q[38],q[7];
cp(pi/1073741824) q[37],q[7];
cp(pi/536870912) q[36],q[7];
cp(pi/268435456) q[35],q[7];
cp(pi/134217728) q[34],q[7];
cp(pi/67108864) q[33],q[7];
cp(pi/33554432) q[32],q[7];
cp(pi/16777216) q[31],q[7];
cp(pi/8388608) q[30],q[7];
cp(pi/4194304) q[29],q[7];
cp(pi/2097152) q[28],q[7];
cp(pi/1048576) q[27],q[7];
cp(pi/524288) q[26],q[7];
cp(pi/262144) q[25],q[7];
cp(pi/131072) q[24],q[7];
cp(pi/65536) q[23],q[7];
cp(pi/32768) q[22],q[7];
cp(pi/16384) q[21],q[7];
cp(pi/8192) q[20],q[7];
cp(pi/4096) q[19],q[7];
cp(pi/2048) q[18],q[7];
cp(pi/1024) q[17],q[7];
cp(pi/512) q[16],q[7];
cp(pi/256) q[15],q[7];
cp(pi/128) q[14],q[7];
cp(pi/64) q[13],q[7];
cp(pi/32) q[12],q[7];
cp(pi/16) q[11],q[7];
cp(pi/8) q[10],q[7];
cp(pi/4) q[9],q[7];
cp(pi/2) q[8],q[7];
h q[7];
cp(0) q[49],q[6];
cp(0) q[48],q[6];
cp(pi/2199023255552.0) q[47],q[6];
cp(pi/1099511627776.0) q[46],q[6];
cp(pi/549755813888.0) q[45],q[6];
cp(pi/274877906944.0) q[44],q[6];
cp(pi/137438953472.0) q[43],q[6];
cp(pi/68719476736.0) q[42],q[6];
cp(pi/34359738368.0) q[41],q[6];
cp(pi/17179869184.0) q[40],q[6];
cp(pi/8589934592.0) q[39],q[6];
cp(pi/4294967296.0) q[38],q[6];
cp(pi/2147483648.0) q[37],q[6];
cp(pi/1073741824) q[36],q[6];
cp(pi/536870912) q[35],q[6];
cp(pi/268435456) q[34],q[6];
cp(pi/134217728) q[33],q[6];
cp(pi/67108864) q[32],q[6];
cp(pi/33554432) q[31],q[6];
cp(pi/16777216) q[30],q[6];
cp(pi/8388608) q[29],q[6];
cp(pi/4194304) q[28],q[6];
cp(pi/2097152) q[27],q[6];
cp(pi/1048576) q[26],q[6];
cp(pi/524288) q[25],q[6];
cp(pi/262144) q[24],q[6];
cp(pi/131072) q[23],q[6];
cp(pi/65536) q[22],q[6];
cp(pi/32768) q[21],q[6];
cp(pi/16384) q[20],q[6];
cp(pi/8192) q[19],q[6];
cp(pi/4096) q[18],q[6];
cp(pi/2048) q[17],q[6];
cp(pi/1024) q[16],q[6];
cp(pi/512) q[15],q[6];
cp(pi/256) q[14],q[6];
cp(pi/128) q[13],q[6];
cp(pi/64) q[12],q[6];
cp(pi/32) q[11],q[6];
cp(pi/16) q[10],q[6];
cp(pi/8) q[9],q[6];
cp(pi/4) q[8],q[6];
cp(pi/2) q[7],q[6];
h q[6];
cp(0) q[49],q[5];
cp(0) q[48],q[5];
cp(0) q[47],q[5];
cp(pi/2199023255552.0) q[46],q[5];
cp(pi/1099511627776.0) q[45],q[5];
cp(pi/549755813888.0) q[44],q[5];
cp(pi/274877906944.0) q[43],q[5];
cp(pi/137438953472.0) q[42],q[5];
cp(pi/68719476736.0) q[41],q[5];
cp(pi/34359738368.0) q[40],q[5];
cp(pi/17179869184.0) q[39],q[5];
cp(pi/8589934592.0) q[38],q[5];
cp(pi/4294967296.0) q[37],q[5];
cp(pi/2147483648.0) q[36],q[5];
cp(pi/1073741824) q[35],q[5];
cp(pi/536870912) q[34],q[5];
cp(pi/268435456) q[33],q[5];
cp(pi/134217728) q[32],q[5];
cp(pi/67108864) q[31],q[5];
cp(pi/33554432) q[30],q[5];
cp(pi/16777216) q[29],q[5];
cp(pi/8388608) q[28],q[5];
cp(pi/4194304) q[27],q[5];
cp(pi/2097152) q[26],q[5];
cp(pi/1048576) q[25],q[5];
cp(pi/524288) q[24],q[5];
cp(pi/262144) q[23],q[5];
cp(pi/131072) q[22],q[5];
cp(pi/65536) q[21],q[5];
cp(pi/32768) q[20],q[5];
cp(pi/16384) q[19],q[5];
cp(pi/8192) q[18],q[5];
cp(pi/4096) q[17],q[5];
cp(pi/2048) q[16],q[5];
cp(pi/1024) q[15],q[5];
cp(pi/512) q[14],q[5];
cp(pi/256) q[13],q[5];
cp(pi/128) q[12],q[5];
cp(pi/64) q[11],q[5];
cp(pi/32) q[10],q[5];
cp(pi/16) q[9],q[5];
cp(pi/8) q[8],q[5];
cp(pi/4) q[7],q[5];
cp(pi/2) q[6],q[5];
h q[5];
cp(0) q[49],q[4];
cp(0) q[48],q[4];
cp(0) q[47],q[4];
cp(0) q[46],q[4];
cp(pi/2199023255552.0) q[45],q[4];
cp(pi/1099511627776.0) q[44],q[4];
cp(pi/549755813888.0) q[43],q[4];
cp(pi/274877906944.0) q[42],q[4];
cp(pi/137438953472.0) q[41],q[4];
cp(pi/68719476736.0) q[40],q[4];
cp(pi/34359738368.0) q[39],q[4];
cp(pi/17179869184.0) q[38],q[4];
cp(pi/8589934592.0) q[37],q[4];
cp(pi/4294967296.0) q[36],q[4];
cp(pi/2147483648.0) q[35],q[4];
cp(pi/1073741824) q[34],q[4];
cp(pi/536870912) q[33],q[4];
cp(pi/268435456) q[32],q[4];
cp(pi/134217728) q[31],q[4];
cp(pi/67108864) q[30],q[4];
cp(pi/33554432) q[29],q[4];
cp(pi/16777216) q[28],q[4];
cp(pi/8388608) q[27],q[4];
cp(pi/4194304) q[26],q[4];
cp(pi/2097152) q[25],q[4];
cp(pi/1048576) q[24],q[4];
cp(pi/524288) q[23],q[4];
cp(pi/262144) q[22],q[4];
cp(pi/131072) q[21],q[4];
cp(pi/65536) q[20],q[4];
cp(pi/32768) q[19],q[4];
cp(pi/16384) q[18],q[4];
cp(pi/8192) q[17],q[4];
cp(pi/4096) q[16],q[4];
cp(pi/2048) q[15],q[4];
cp(pi/1024) q[14],q[4];
cp(pi/512) q[13],q[4];
cp(pi/256) q[12],q[4];
cp(pi/128) q[11],q[4];
cp(pi/64) q[10],q[4];
cp(pi/32) q[9],q[4];
cp(pi/16) q[8],q[4];
cp(pi/8) q[7],q[4];
cp(pi/4) q[6],q[4];
cp(pi/2) q[5],q[4];
h q[4];
cp(0) q[49],q[3];
cp(0) q[48],q[3];
cp(0) q[47],q[3];
cp(0) q[46],q[3];
cp(0) q[45],q[3];
cp(pi/2199023255552.0) q[44],q[3];
cp(pi/1099511627776.0) q[43],q[3];
cp(pi/549755813888.0) q[42],q[3];
cp(pi/274877906944.0) q[41],q[3];
cp(pi/137438953472.0) q[40],q[3];
cp(pi/68719476736.0) q[39],q[3];
cp(pi/34359738368.0) q[38],q[3];
cp(pi/17179869184.0) q[37],q[3];
cp(pi/8589934592.0) q[36],q[3];
cp(pi/4294967296.0) q[35],q[3];
cp(pi/2147483648.0) q[34],q[3];
cp(pi/1073741824) q[33],q[3];
cp(pi/536870912) q[32],q[3];
cp(pi/268435456) q[31],q[3];
cp(pi/134217728) q[30],q[3];
cp(pi/67108864) q[29],q[3];
cp(pi/33554432) q[28],q[3];
cp(pi/16777216) q[27],q[3];
cp(pi/8388608) q[26],q[3];
cp(pi/4194304) q[25],q[3];
cp(pi/2097152) q[24],q[3];
cp(pi/1048576) q[23],q[3];
cp(pi/524288) q[22],q[3];
cp(pi/262144) q[21],q[3];
cp(pi/131072) q[20],q[3];
cp(pi/65536) q[19],q[3];
cp(pi/32768) q[18],q[3];
cp(pi/16384) q[17],q[3];
cp(pi/8192) q[16],q[3];
cp(pi/4096) q[15],q[3];
cp(pi/2048) q[14],q[3];
cp(pi/1024) q[13],q[3];
cp(pi/512) q[12],q[3];
cp(pi/256) q[11],q[3];
cp(pi/128) q[10],q[3];
cp(pi/64) q[9],q[3];
cp(pi/32) q[8],q[3];
cp(pi/16) q[7],q[3];
cp(pi/8) q[6],q[3];
cp(pi/4) q[5],q[3];
cp(pi/2) q[4],q[3];
h q[3];
cp(0) q[49],q[2];
cp(0) q[48],q[2];
cp(0) q[47],q[2];
cp(0) q[46],q[2];
cp(0) q[45],q[2];
cp(0) q[44],q[2];
cp(pi/2199023255552.0) q[43],q[2];
cp(pi/1099511627776.0) q[42],q[2];
cp(pi/549755813888.0) q[41],q[2];
cp(pi/274877906944.0) q[40],q[2];
cp(pi/137438953472.0) q[39],q[2];
cp(pi/68719476736.0) q[38],q[2];
cp(pi/34359738368.0) q[37],q[2];
cp(pi/17179869184.0) q[36],q[2];
cp(pi/8589934592.0) q[35],q[2];
cp(pi/4294967296.0) q[34],q[2];
cp(pi/2147483648.0) q[33],q[2];
cp(pi/1073741824) q[32],q[2];
cp(pi/536870912) q[31],q[2];
cp(pi/268435456) q[30],q[2];
cp(pi/134217728) q[29],q[2];
cp(pi/67108864) q[28],q[2];
cp(pi/33554432) q[27],q[2];
cp(pi/16777216) q[26],q[2];
cp(pi/8388608) q[25],q[2];
cp(pi/4194304) q[24],q[2];
cp(pi/2097152) q[23],q[2];
cp(pi/1048576) q[22],q[2];
cp(pi/524288) q[21],q[2];
cp(pi/262144) q[20],q[2];
cp(pi/131072) q[19],q[2];
cp(pi/65536) q[18],q[2];
cp(pi/32768) q[17],q[2];
cp(pi/16384) q[16],q[2];
cp(pi/8192) q[15],q[2];
cp(pi/4096) q[14],q[2];
cp(pi/2048) q[13],q[2];
cp(pi/1024) q[12],q[2];
cp(pi/512) q[11],q[2];
cp(pi/256) q[10],q[2];
cp(pi/128) q[9],q[2];
cp(pi/64) q[8],q[2];
cp(pi/32) q[7],q[2];
cp(pi/16) q[6],q[2];
cp(pi/8) q[5],q[2];
cp(pi/4) q[4],q[2];
cp(pi/2) q[3],q[2];
h q[2];
cp(0) q[49],q[1];
cp(0) q[48],q[1];
cp(0) q[47],q[1];
cp(0) q[46],q[1];
cp(0) q[45],q[1];
cp(0) q[44],q[1];
cp(0) q[43],q[1];
cp(pi/2199023255552.0) q[42],q[1];
cp(pi/1099511627776.0) q[41],q[1];
cp(pi/549755813888.0) q[40],q[1];
cp(pi/274877906944.0) q[39],q[1];
cp(pi/137438953472.0) q[38],q[1];
cp(pi/68719476736.0) q[37],q[1];
cp(pi/34359738368.0) q[36],q[1];
cp(pi/17179869184.0) q[35],q[1];
cp(pi/8589934592.0) q[34],q[1];
cp(pi/4294967296.0) q[33],q[1];
cp(pi/2147483648.0) q[32],q[1];
cp(pi/1073741824) q[31],q[1];
cp(pi/536870912) q[30],q[1];
cp(pi/268435456) q[29],q[1];
cp(pi/134217728) q[28],q[1];
cp(pi/67108864) q[27],q[1];
cp(pi/33554432) q[26],q[1];
cp(pi/16777216) q[25],q[1];
cp(pi/8388608) q[24],q[1];
cp(pi/4194304) q[23],q[1];
cp(pi/2097152) q[22],q[1];
cp(pi/1048576) q[21],q[1];
cp(pi/524288) q[20],q[1];
cp(pi/262144) q[19],q[1];
cp(pi/131072) q[18],q[1];
cp(pi/65536) q[17],q[1];
cp(pi/32768) q[16],q[1];
cp(pi/16384) q[15],q[1];
cp(pi/8192) q[14],q[1];
cp(pi/4096) q[13],q[1];
cp(pi/2048) q[12],q[1];
cp(pi/1024) q[11],q[1];
cp(pi/512) q[10],q[1];
cp(pi/256) q[9],q[1];
cp(pi/128) q[8],q[1];
cp(pi/64) q[7],q[1];
cp(pi/32) q[6],q[1];
cp(pi/16) q[5],q[1];
cp(pi/8) q[4],q[1];
cp(pi/4) q[3],q[1];
cp(pi/2) q[2],q[1];
h q[1];
cp(0) q[49],q[0];
cp(0) q[48],q[0];
cp(0) q[47],q[0];
cp(0) q[46],q[0];
cp(0) q[45],q[0];
cp(0) q[44],q[0];
cp(0) q[43],q[0];
cp(0) q[42],q[0];
cp(pi/2199023255552.0) q[41],q[0];
cp(pi/1099511627776.0) q[40],q[0];
cp(pi/549755813888.0) q[39],q[0];
cp(pi/274877906944.0) q[38],q[0];
cp(pi/137438953472.0) q[37],q[0];
cp(pi/68719476736.0) q[36],q[0];
cp(pi/34359738368.0) q[35],q[0];
cp(pi/17179869184.0) q[34],q[0];
cp(pi/8589934592.0) q[33],q[0];
cp(pi/4294967296.0) q[32],q[0];
cp(pi/2147483648.0) q[31],q[0];
cp(pi/1073741824) q[30],q[0];
cp(pi/536870912) q[29],q[0];
cp(pi/268435456) q[28],q[0];
cp(pi/134217728) q[27],q[0];
cp(pi/67108864) q[26],q[0];
cp(pi/33554432) q[25],q[0];
cp(pi/16777216) q[24],q[0];
cp(pi/8388608) q[23],q[0];
cp(pi/4194304) q[22],q[0];
cp(pi/2097152) q[21],q[0];
cp(pi/1048576) q[20],q[0];
cp(pi/524288) q[19],q[0];
cp(pi/262144) q[18],q[0];
cp(pi/131072) q[17],q[0];
cp(pi/65536) q[16],q[0];
cp(pi/32768) q[15],q[0];
cp(pi/16384) q[14],q[0];
cp(pi/8192) q[13],q[0];
cp(pi/4096) q[12],q[0];
cp(pi/2048) q[11],q[0];
cp(pi/1024) q[10],q[0];
cp(pi/512) q[9],q[0];
cp(pi/256) q[8],q[0];
cp(pi/128) q[7],q[0];
cp(pi/64) q[6],q[0];
cp(pi/32) q[5],q[0];
cp(pi/16) q[4],q[0];
cp(pi/8) q[3],q[0];
cp(pi/4) q[2],q[0];
cp(pi/2) q[1],q[0];
h q[0];
swap q[0],q[49];
swap q[1],q[48];
swap q[2],q[47];
swap q[3],q[46];
swap q[4],q[45];
swap q[5],q[44];
swap q[6],q[43];
swap q[7],q[42];
swap q[8],q[41];
swap q[9],q[40];
swap q[10],q[39];
swap q[11],q[38];
swap q[12],q[37];
swap q[13],q[36];
swap q[14],q[35];
swap q[15],q[34];
swap q[16],q[33];
swap q[17],q[32];
swap q[18],q[31];
swap q[19],q[30];
swap q[20],q[29];
swap q[21],q[28];
swap q[22],q[27];
swap q[23],q[26];
swap q[24],q[25];